...
class restore_qtype_translateandspeak_plugin extends restore_qtype_plugin {
    /**
    ...
    protected function define_question_plugin_structure() {
        ...
        // Add own qtype stuff.
        $paths[] = new restore_path_element(self::qtype_name(), $this->get_pathfor('/translateandspeak'));

        return $paths;
    }

    /**
     * Process the qtype/translateandspeak element
     *
     * @param array $data
     */
    public function process_translateandspeak($data) {
        global $DB;
        ...
        // we need to create a "qtype_tas_options" record
        // and create a mapping from the $oldid to the $newid.
        if ($this->get_mappingid('question_created', $oldquestionid)) {
            $data->questionid = $newquestionid;
            $newid = $DB->insert_record('qtype_tas_options', $data);
            $this->set_mapping('qtype_tas_options', $oldid, $newid);
        }
    }
}
